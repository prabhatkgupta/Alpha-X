...
def generate_detailed_monthly_summary(df):
    ...
    if 'coding' in df.columns:
        ...
        if coding_rate >= 85:
            report_lines.append(f"✅ Coding: {coding_yes}/{days_tracked} days ({coding_rate:.0f}%) - Outstanding!")
        elif coding_rate >= 70:
            report_lines.append(f"✅ Coding: {coding_yes}/{days_tracked} days ({coding_rate:.0f}%) - Good consistency")
        else:
            report_lines.append(f"⚠️ Coding: {coding_yes}/{days_tracked} days ({coding_rate:.0f}%) - Needs improvement")
        
        if trends and 'coding' in trends:
            change = trends['coding']['change']
            if change > 0:
                report_lines.append(f"📈 Trend: +{change} days from start to end of month (Improving!)")
            elif change < 0:
                report_lines.append(f"📉 Trend: {change} days from start to end of month (Declining)")
            else:
    ...
    if 'focus' in df.columns:
        ...
        report_lines.append(f"🎯 Focus: {sharp_days} days sharp ({sharp_rate:.0f}%), {multitask_days} days multi-tasking")
    ...
    if 'protein' in df.columns:
        ...
        report_lines.append(f"🍗 Protein: {protein_met}/{days_tracked} days ({protein_rate:.0f}%) met 100g target")
    ...
    if 'workout' in df.columns:
        ...
        report_lines.append(f"🏋️ Workouts: {workout_days}/{days_tracked} days ({workout_rate:.0f}%)")
        ...
        if trends and 'workout' in trends:
            ...
            if change > 0:
                ...
                report_lines.append(f"📉 Workout frequency decreased by {abs(change)} days")
    ...
    if 'sleep' in df.columns:
        ...
        if sleep_hours:
            ...
            report_lines.append(f"😴 Sleep: Avg {avg_sleep:.1f} hrs/night, {ideal_nights} nights in ideal range (7-8 hrs)")
    ...
    if 'marriage' in df.columns:
        ...
        report_lines.append(f"💑 Good: {good_days} days ({good_rate:.0f}%), Okayish: {okayish_days}, Not good: {not_good_days}")
    ...
    if 'happiness' in df.columns:
        ...
        report_lines.append(f"😊 Happy Days: {happy}/{days_tracked} ({happy_rate:.0f}%)")
    ...
    if 'performance' in df.columns:
        ...
        report_lines.append(f"📊 Better Days: {better} ({better_rate:.0f}%), Worse: {worse}")
    ...
    if 'day_overview' in df.columns:
        ...
        report_lines.append(f"💼 Work Quality: {hard_enjoyed} days enjoyed, {procrastinated} procrastinated, {burnout} burned out")
    ...
    # Top achievement
    achievements = []
    if 'protein' in df.columns and (df['protein'] == '>= 100g').sum() / days_tracked >= 0.8:
    ...
    improvements = []
    if 'sleep' in df.columns:
        sleep_hours = [int(re.search(r'(\d+)', str(v)).group(1)) for v in df['sleep'] if isinstance(v, str) and re.search(r'(\d+)', str(v))]
        if sleep_hours and sum(sleep_hours) / len(sleep_hours) < 7:
            improvements.append("😴 Sleep - aim for 7-8 hours consistently")
    
    if 'focus' in df.columns:
        if (df['focus'] == "I was multi-tasking, not good focus").sum() > (df['focus'] == 'Good, razor sharp').sum():
