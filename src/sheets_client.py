...
class SheetsClient:
    ...
    def get_weekly_data(self, weeks_ago: int = 0) -> pd.DataFrame:
        ...
        mask = (df['timestamp'] >= start_of_target_week) & (df['timestamp'] <= end_of_target_week)
        weekly_df = df[mask].copy()
        
        print(f"📅 Data for week: {start_of_target_week.date()} to {end_of_target_week.date()}")
        print(f"📊 Found {len(weekly_df)} entries")
        
        return weekly_df
    
    def get_date_range_data(self, start_date: datetime, end_date: datetime) -> pd.DataFrame:
        """
